...
class RsyncRdiff(BackupExecution):
  ...
  def execute(self):
    ...
    e('ssh %s sudo tar --remove-files --overwrite zxvf %s /tmp/%s.tar.gz' % (c['host'], c['directory'], backup))
    e('rsync -e ssh test1:/tmp/%s.tar.gz %s/%s.tar.gz' % (backup, self.tmp_dir, backup))
